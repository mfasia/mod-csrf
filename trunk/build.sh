...
./configure --with-mpm=worker --enable-so --enable-csrf=shared --enable-proxy=shared --enable-ssl --enable-status=shared --enable-info=shared --enable-static-support --enable-unique-id --enable-dumpio=shared $ADDMOD
if [ $? -ne 0 ]; then
  echo "ERROR"
  exit 1
fi
